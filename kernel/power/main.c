...
#ifdef CONFIG_MACH_LOGAN
extern void wakelock_suspend(int opt);
static ssize_t release_wakelock_show(struct kobject *kobj,
				struct kobj_attribute *attr,
				char *buf)
{
	return -EINVAL;
...
static ssize_t release_wakelock_store(struct kobject *kobj,
				struct kobj_attribute *attr,
				const char *buf, size_t n)
{
	unsigned int ret;

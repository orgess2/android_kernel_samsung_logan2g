...
unsigned int mmc_cmd_err;
unsigned int sd_cmd_err;
			 ...
			 int type)
{
	...
	retries = 5;
    do{
  printk("mmc_blk_reset retries=%x\n",retries);
			     ...
			     struct mmc_async_req *areq)
{
	...
	if (brq->cmd.resp[0] & CMD_ERRORS) {
		printk("%s: r/w command failed, status = %#x\n",
...
static void mmc_blk_remove_req(struct mmc_blk_data *md)
{
	...
	if (md) {
		if (md->disk->flags & GENHD_FL_UP) {
			...
			WARN_ON(1);
			printk("****** %s call del_gendisk_async *****\n", __func__ );

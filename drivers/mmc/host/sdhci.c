...
static u32 sdhci_write_block_count[2] = {0};
static void sdhci_update_debug_info(struct sdhci_host *host, u32 count)
{
	u32 idx = 0;
	...
	sdhci_write_block_count[idx]  = sdhci_write_block_count[idx] + count ;
}
...
static void sdhci_send_command(struct sdhci_host *host, struct mmc_command *cmd)
{
	...
	/* Wait max 10 ms */
	timeout = 10;
	if( MMC_WRITE_BLOCK == cmd->opcode )
		sdhci_update_debug_info(host,1);
	if( MMC_WRITE_MULTIPLE_BLOCK == cmd->opcode )
		sdhci_update_debug_info(host,cmd->data->blocks);	
...
static int sdhci_debug_show(struct seq_file *s, void *data)
{
	u32 i;
	for(i = 0;i < 2; i++) {
		seq_printf(s, "mmc%d sdhci_write_block_count 0x%08x\n",i, sdhci_write_block_count[i]);
	}
	return 0;
}
